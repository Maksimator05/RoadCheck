...
По умолчанию backend запускается в mock-режиме.

Чтобы включить реальную модель:

1. Положите веса модели в `backend/app/ml/weights/best.pt` или укажите свой путь в `ML_MODEL_PATH`.
2. В `backend/.env` установите `USE_MOCK_ML=false`.
3. Перезапустите backend. На старте приложение попытается загрузить модель сразу и не запустится, если веса не найдены или не читаются.
...
Для запуска compose с реальной моделью положите веса в `backend/app/ml/weights/` и задайте `USE_MOCK_ML=false` в окружении или в `backend/.env`.
